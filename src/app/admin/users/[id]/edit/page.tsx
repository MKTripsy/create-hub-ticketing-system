...
export default function EditUserPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await createNotification('user_edited', `Admin updated user: ${form.first_name} ${form.last_name}`)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await createNotification('user_deleted', `Admin deleted user: ${form.first_name} ${form.last_name}`)
...
//   // Fetch user data — remove time_slots from here
//   useEffect(() => {
//     const fetchData = async () => {
//       const [userRes, spacesRes, availRes] = await Promise.all([
//         supabase.from('users').select('*').eq('id', numericId).single(),
//         supabase.from('spaces').select('*').eq('is_active', true),
...
//   // Fetch limits when space changes — keep this exactly as is
//   useEffect(() => {
//     if (!form.primary_space_id) return
...
//     }
//     fetchLimits()
//   }, [form.primary_space_id, numericId])
